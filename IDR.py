...
# Fungsi untuk ekspor transaksi ke CSV
def export_transactions_to_csv(user_data):
    if 'transactions' not in user_data:
        return None

    # Membuat file CSV dalam buffer StringIO (untuk text)
    csv_buffer = io.StringIO()
    
    # Membuat writer untuk menulis ke dalam buffer
    writer = csv.DictWriter(csv_buffer, fieldnames=['date', 'type', 'category', 'amount'])
    writer.writeheader()
    
    # Menulis semua transaksi ke CSV
    for record in user_data['transactions']:
        writer.writerow(record)
    
    # Ambil konten dari buffer dan konversi ke bytes
    csv_content = csv_buffer.getvalue().encode('utf-8')
    
    # Membuat buffer BytesIO untuk mengirimkan file
    csv_bytes_io = io.BytesIO(csv_content)
    csv_bytes_io.seek(0)  # Kembali ke awal buffer
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    help_text = (
        ...
        "/exportcsv - Ekspor transaksi ke file CSV\n"
    )
...
async def uang_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if transaction_type == 'keluar' and 'budgets' in context.user_data and category in context.user_data['budgets']:
        ...
        if budget >= amount:
            ...
            await update.message.reply_text(f"Transaksi {transaction_type} kategori '{category}' sebesar IDR {format_currency(amount)} dicatat. Sisa anggaran untuk '{category}': IDR {format_currency(context.user_data['budgets'][category])}.")
        else:
            await update.message.reply_text(f"Anggaran untuk kategori '{category}' tidak cukup untuk pengeluaran sebesar IDR {format_currency(amount)}.")
    else:
        await update.message.reply_text(f"Transaksi {transaction_type} kategori '{category}' sebesar IDR {format_currency(amount)} dicatat.")
...
async def report_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    report = (
        ...
        f"Total Pendapatan: IDR {format_currency(masuk)}\n"
        f"Total Pengeluaran: IDR {format_currency(keluar)}\n"
        f"Sisa: IDR {format_currency(profit_loss)}\n\n"
        "Detail Transaksi:\n"
    )
    ...
    if 'transactions' in context.user_data:
        for record in context.user_data['transactions']:
            ...
            report += (f"{transaction_time} - {transaction_type} - {category}: IDR {amount}\n")
    else:
...
# Fungsi untuk ekspor transaksi ke file CSV
async def export_csv_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    csv_file = export_transactions_to_csv(context.user_data)
    if csv_file:
        await update.message.reply_document(document=csv_file, filename="transactions.csv")
    else:
...
async def set_budget_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    context.user_data['budgets'][category] = amount
    await update.message.reply_text(f"Anggaran untuk kategori '{category}' telah diset sebesar IDR {format_currency(amount)}.")
...
async def check_budget_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for category, amount in context.user_data['budgets'].items():
        budget_report += f"{category.capitalize()}: IDR {format_currency(amount)}\n"
...
if __name__ == '__main__':
    application = ApplicationBuilder().token(
        "7548323996:AAG7KMn4Bg8VeVkdtkJmYXhFeNOVvOH3T24").build()
